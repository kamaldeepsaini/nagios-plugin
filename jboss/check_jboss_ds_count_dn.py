...
cmd_parser.add_option("-S", "--server", type="string", action="store", dest="jboss_server", help="JBOSS Server Name, e.g localhost")
cmd_parser.add_option("-P", "--port", type="string", action="store", dest="jboss_port", help="JBOSS Server Port, e.g 9990")
cmd_parser.add_option("-H", "--host", type="string", action="store", dest="jboss_host", help="JBOSS Server host, e.g master,slave ")
cmd_parser.add_option("-s", "--servername", type="string", action="store", dest="jboss_servername", help="JBOSS Server type, e.g  server-three")
...
if not (cmd_options.jboss_server and cmd_options.jboss_host and cmd_options.jboss_servername and cmd_options.jboss_port and cmd_options.jboss_user and cmd_options.jboss_pwd and cmd_options.jboss_dstype and cmd_options.jboss_dsname and cmd_options.jboss_warn and cmd_options.jboss_critical):
...
url = "http://"+cmd_options.jboss_user+":"+cmd_options.jboss_pwd+"@"+cmd_options.jboss_server+":"+cmd_options.jboss_port+"/management/host/"+cmd_options.jboss_host+"/server/"+cmd_options.jboss_servername+"/subsystem/datasources/"+cmd_options.jboss_dstype+"/"+cmd_options.jboss_dsname+"/statistics/pool?"+urllib.urlencode({'include-runtime': 'true'})
